...
<?php
	...
	$photoid = 0;
	$proofofaddress = 0;
	if(array_key_exists('newsletter', $all_details) &&
		array_key_exists('gender', $all_details) && 
		array_key_exists('aboutme', $all_details) && 
		array_key_exists('usertype', $all_details) && 
		array_key_exists('user_image', $all_details)
		){
		$profile_ = 20;
	}
	if(array_key_exists('_company_twitter', $all_details)){
		$socialprofile = 20;
	}
	if(array_key_exists('carertags', $all_details) || array_key_exists('parentags', $all_details) ){
		$tags = 20;
	}
	if(array_key_exists('user_image', $all_details)){
		$photoid = 20;
	}
	if(array_key_exists('file', $all_details)){
		$proofofaddress = 20;
	}
	$total = $profile_ + $socialprofile + $tags + $photoid + $proofofaddress;
	?>
	<p>
		<span class="password-change-title"><?php _e( 'Profile Verification', 'woocommerce' ); ?></span>
		<i class="fa fa-exclamation-triangle"></i>
		<?php
			if($total != 100){
		 		_e( 'Complete all point to earn Irishcare Seal of Approval on you Account.', 'woocommerce' );
			}
		?>
	</p>		
</div>
<p class="form-row form-row-half">
	<?php
	if($results[usertype][0] == 'looking_services'){ ?>
		<h3 class="password-change-title"><?php _e( 'Only For Parents &nbsp;'.$total.'%', 'woocommerce' ); 		
	}else{ ?>
		<h3 class="password-change-title"><?php _e( 'Only For Care Givers &nbsp;'.$total.'%', 'woocommerce' );
	}
	?>
		<table border="1">
			<tr>
			...
			<tr>
				<div class="progess">
		        	<p style="width:<?= $total; ?>%; border-top:4px solid #c3c3c3; width:100%;"></p>
				</div>
			</tr>
	   </table>
	   <style type="text/css">
	   .progess p:before, .progess p:after{
	   		width: <?= $total+5; ?>%;
	   }
	   </style>
	</h3>
	<ul>
		<li>1.Complete Your Profile
			<b>+<?php echo $profile_.'%';
	   			if($results['is_profile_approved'][0] == 1){
					echo '<span class="done"> Approved </span>';
	   			}elseif($profile_ == 20){
		   			echo '<span class="done"> Done </span>';
		   		}else{
					echo '<span class="await"> Awaiting </span>';
				}?>
			</b>
		</li>
		<li>2.Connect Your Social Profile
			<b>+<?php echo $socialprofile.'%';
				if($results['is_socialprofile_approved'][0] == 1){
					echo '<span class="done"> Approved </span>';
	   			}elseif($socialprofile == 20){
		   			echo '<span class="done"> Done </span>';
		   		}else{
					echo '<span class="await"> Awaiting </span>';
				}?>
		...
		<li>3.Complete Your Tags
			<b>+<?php echo $tags.'%';
				if($results['is_tags_approved'][0] == 1){
					echo '<span class="done"> Approved </span>';
	   			}elseif($tags == 20){
		   			echo '<span class="done"> Done </span>';
		   		}else{
		...
		<li>4.Upload Your Photo ID
			<b>+<?php echo $photoid.'%';
				if($results['is_photoid_approved'][0] == 1){
					echo '<span class="done"> Approved </span>';
	   			}elseif($photoid == 20){
		   			echo '<span class="done"> Done </span>';
		   		}else{
		...
		<li>5.Garda Clearnce Certificate
			<b>+<?php echo $proofofaddress.'%';
				if($results['is_proofofaddress_approved'][0] == 1){
					echo '<span class="done"> Approved </span>';
	   			}elseif($proofofaddress == 20){
		   			echo '<span class="done"> Done </span>';
		   		}else{
					echo '<span class="await"> Awaiting </span>';
				}?>
...
<form class="account_carer" action="<?= get_site_url(); ?>/my-account/view-account/" method="post" enctype="multipart/form-data">
	...
	<p class="form-row form-row-half">
		...
		?>
		<input class="carer-input-text" type="file" name="user_image">
	</p>
	<p class="form-row form-row-half">
